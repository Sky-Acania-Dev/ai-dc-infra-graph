import React, { createContext, useContext, useMemo, useState } from "react";

export type Locale = "en" | "zh-CN";

type TranslationParams = Record<string, string | number>;
type I18nContextValue = {
  locale: Locale;
  setLocale: (locale: Locale) => void;
  t: (key: string, params?: TranslationParams) => string;
  formatNumber: (value: number) => string;
  formatPercent: (value: number) => string;
  formatConstructionPhase: (phase: string) => string;
  formatLifecycleStatus: (status: string) => string;
  formatCableStatus: (status: string) => string;
  formatCableProgressStep: (step: string) => string;
  formatCableProgressState: (state: string) => string;
  formatCableProgressPhaseName: (phase: string) => string;
  formatCableProgressPhaseType: (phaseType: string) => string;
  formatValidationClassification: (classification: string) => string;
};

const STORAGE_KEY = "ai-dc-infra-graph-locale";

const TRANSLATIONS: Record<Locale, Record<string, string>> = {
  en: {
    "app.name": "AI DC Infra Graph",
    "app.topologyTitle": "Infrastructure Topology",
    "app.validationTitle": "Validation Results",
    "view.topology": "Topology",
    "view.validation": "Validation",
    "settings.label": "Settings",
    "settings.language": "Language",
    "settings.english": "English",
    "settings.chineseSimplified": "Chinese Simplified",
    "auth.loading": "Role...",
    "auth.role.manager": "Manager",
    "auth.role.editor": "Editor",
    "auth.role.viewer": "Viewer",
    "dataHall.selector": "Data hall",
    "dataHall.fallback": "Data Hall",
    "dataHall.summary": "Data Hall Summary",
    "common.loading": "Loading {{target}}",
    "common.close": "Close",
    "common.filter": "Filter",
    "common.rows": "Rows",
    "common.go": "Go",
    "common.map": "Map",
    "common.count": "Count",
    "common.finding": "Finding",
    "common.examples": "Examples",
    "common.status": "Status",
    "common.group": "Group",
    "common.type": "Type",
    "common.port": "Port",
    "common.side": "Side",
    "common.model": "Model",
    "common.normalized": "Normalized",
    "common.noFindings": "No findings.",
    "common.noExamples": "No row examples available.",
    "viewer.scale": "Scale",
    "viewer.tableScale": "Table text",
    "cabinet.details": "Cabinet Details",
    "cabinet.cabinets": "Cabinets",
    "cabinet.category": "Type",
    "cabinet.categories": "Categories",
    "cabinet.group": "Group",
    "cabinet.unassigned": "Unassigned",
    "cabinet.status": "Status",
    "cabinet.constructionPhase": "Construction Phase",
    "cabinet.rackUnits": "Rack Units",
    "cabinet.devices": "Devices",
    "cabinet.ports": "Ports",
    "cabinet.cables": "Cables",
    "cabinet.types": "Cabinet Types",
    "bulkStatus.setStatus": "Set status",
    "bulkStatus.chooseStatus": "Set {{count}} selected...",
    "map.cabinetMap": "Cabinet Map",
    "portLayout.title": "Port Layout",
    "portLayout.mode": "Map mode",
    "portLayout.eyebrow": "Device Port Layout",
    "portLayout.front": "Front",
    "portLayout.back": "Back",
    "portLayout.noDevice": "No device selected",
    "portLayout.selectDevice": "Select a device to view its port layout.",
    "portLayout.noBackPorts": "No back-side port layout configured.",
    "map.size": "Map size",
    "map.size.xsmall": "XS",
    "map.size.small": "S",
    "map.size.medium": "M",
    "map.size.large": "L",
    "map.size.xlarge": "XL",
    "rack.units": "Rack Units",
    "rack.unitLabel": "U{{unit}}",
    "device.portsCount": "{{count}} ports",
    "device.statusAndPorts": "{{status}} / {{count}} ports",
    "device.viewPortLayout": "Port layout",
    "connections.connectedDevices": "Connected Devices",
    "connections.connectedDevicesCount": "{{count}} Connected Devices",
    "connections.connectedCabinets": "Connected Cabinets",
    "connections.connectedCabinetsCount": "{{count}} Connected Cabinets",
    "connections.emptyCabinets": "Connected cabinets will appear here after selection.",
    "connections.dataHallCables": "Data Hall Cables",
    "connections.insideDataHall": "Inside {{dataHall}}",
    "connections.intraDataHall": "Cables with both ends inside this data hall",
    "connections.totalExternal": "Total external",
    "connections.totalExternalSubtitle": "All cables crossing from this data hall to other data halls",
    "connections.totalExternalCabinetSubtitle": "All cables from this cabinet to connected peer cabinets",
    "connections.totalExternalDeviceSubtitle": "All cables from this device to connected devices in other cabinets",
    "connections.selectedCabinets": "{{count}} selected cabinets",
    "connections.selectedDevices": "{{count}} selected devices",
    "connections.intraCabinetSelection": "Device-to-device cables inside the selected cabinets",
    "connections.intraDeviceSelection": "Cables between the selected devices",
    "connections.betweenSelectedCabinets": "Between selected cabinets",
    "connections.selectedCabinetPeerSubtitle": "Cables crossing between selected cabinets",
    "connections.totalExternalCabinetSelectionSubtitle": "All cables from selected cabinets to unselected peer cabinets",
    "connections.totalExternalDeviceSelectionSubtitle": "All cables from selected devices to unselected peer devices",
    "connections.toDataHall": "To {{dataHall}}",
    "connections.crossDataHall": "Cables crossing to another data hall",
    "connections.cableCount": "{{count}} cables",
    "connections.deviceCount": "{{count}} devices",
    "connections.insideCabinet": "Inside {{cabinetUid}}",
    "connections.intraCabinet": "Device-to-device cables within this cabinet",
    "connections.intraDevice": "Connected devices within this cabinet",
    "connections.targetCabinetRu": "Cabinet {{cabinetUid}} / RU {{rackUnit}}",
    "connections.viewCables": "View cables",
    "connections.expandPeers": "Show peer targets ({{count}})",
    "connections.collapsePeers": "Hide peer targets",
    "completion.text": "{{completed}}/{{total}} complete",
    "completion.aria": "Cable completion",
    "cable.details": "Cable Details",
    "cable.routeTitle": "{{source}} to {{target}}",
    "cable.countVisible": "{{visible}} of {{total}} cables",
    "cable.closeDetails": "Close cable details",
    "cable.column.type": "Type",
    "cable.column.status": "Status",
    "cable.column.group": "Group",
    "cable.column.aPort": "A Port",
    "cable.column.zPort": "Z Port",
    "cable.column.aOptic": "A Optic",
    "cable.column.zOptic": "Z Optic",
    "cable.column.uid": "Cable ID",
    "cable.column.lengthMeters": "Length (m)",
    "cable.column.progress": "Progress",
    "cable.column.constructionPhase": "Construction Phase",
    "cable.column.note": "Note",
    "cable.filterAll": "All",
    "cable.filterNone": "None",
    "cable.filterHiddenOn": "{{count}} hidden selected",
    "cable.filterHiddenOff": "{{count}} hidden excluded",
    "cable.phaseType.single_percent": "Single Task",
    "cable.phaseType.parallel_percent": "Parallel Tasks",
    "cable.phaseType.enum_state": "Enum State",
    "cable.phaseName.routing_dress": "Routing Dress",
    "cable.phaseName.termination": "Termination",
    "cable.phaseName.cabinet_dress": "Cabinet Dress",
    "cable.phaseName.final_result": "Final Result",
    "cable.phaseName.preparation": "Preparation",
    "cable.phaseName.dress_termination": "Dress / Termination",
    "cable.phaseName.validation": "Validation",
    "cable.step.preparation": "Preparation",
    "cable.step.ordered": "Ordered",
    "cable.step.received": "Received",
    "cable.step.labeled": "Labeled",
    "cable.step.bundled": "Bundled",
    "cable.step.pulled": "Pulled",
    "cable.step.routing_dress": "Routing Dress",
    "cable.step.a_side": "A-side",
    "cable.step.z_side": "Z-side",
    "cable.step.not_terminated": "Not Terminated",
    "cable.step.terminated": "Terminated",
    "cable.step.dressed": "Dressed",
    "cable.step.validation": "Validation",
    "cable.step.validated": "Validated",
    "cable.step.failed": "Failed",
    "cable.step.broken": "Broken",
    "cabinet.termination": "Termination",
    "cabinet.dress": "Dress",
    "map.display": "Map Display",
    "map.displayText": "Type",
    "map.displayType": "Type",
    "map.displayTermination": "Term",
    "map.displayDress": "Dress",
    "map.loadingData": "Loading data",
    "validation.qa": "QA",
    "validation.results": "Validation Results",
    "validation.portCollisions": "Port Collisions",
    "validation.modelMismatches": "Model Mismatches",
    "validation.formatIssues": "Format Issues",
    "validation.deviceModelMismatches": "Device Model Mismatches",
    "validation.deviceModelFormatIssues": "Device Model Format Issues",
    "validation.device": "Device",
    "validation.models": "Models",
    "validation.findingDetail": "Finding Detail",
    "validation.closeDetail": "Close validation detail",
    "validation.deviceName": "Device Name",
    "validation.aModel": "A Model",
    "validation.zModel": "Z Model",
    "validation.classification.model_mismatch": "Model mismatch",
    "validation.classification.trivial_format_issue": "Format issue",
    "lifecycle.unknown": "Unknown",
    "lifecycle.not_constructed": "Not Constructed",
    "lifecycle.not_installed": "Not Installed",
    "lifecycle.not_powered": "Not Powered",
    "lifecycle.installed": "Installed",
    "lifecycle.powered": "Powered",
    "lifecycle.active": "Active",
    "lifecycle.not_planned": "Not Planned",
    "constructionPhase.Management & Ethernet": "Management & Ethernet",
    "constructionPhase.RoCE": "RoCE",
    "cableStatus.Cable Is Ran: Complete": "Cable Is Ran: Complete",
    "cableStatus.Cable Is Ran: Not Terminated": "Cable Is Ran: Not Terminated",
    "cableStatus.Cable Not Run": "Cable Not Run",
    "cableStatus.Unknown": "Unknown",
    "cableProgressState.not_started": "Not Started",
    "cableProgressState.complete": "Complete",
    "cableProgressState.blocked": "Blocked",
    "cableProgressState.not_applicable": "Not Applicable",
    "cableProgressStep.purchased": "Purchased",
    "cableProgressStep.received": "Received",
    "cableProgressStep.categorized_stored": "Categorized and Stored",
    "cableProgressStep.labeled": "Labeled",
    "cableProgressStep.bundled_on_ground": "Bundled on Ground",
    "cableProgressStep.pulled": "Pulled",
    "cableProgressStep.dressed": "Dressed",
    "cableProgressStep.a_side_terminated": "A Side Terminated",
    "cableProgressStep.z_side_terminated": "Z Side Terminated",
    "cableProgressStep.a_side_dressed_in_cabinet": "A Side Dressed in Cabinet",
    "cableProgressStep.z_side_dressed_in_cabinet": "Z Side Dressed in Cabinet",
    "cableProgressStep.validated": "Validated",
    "cableProgressStep.broken": "Broken",
  },
  "zh-CN": {
    "app.name": "AI 数据中心基础设施图谱",
    "app.topologyTitle": "基础设施拓扑",
    "app.validationTitle": "校验结果",
    "view.topology": "拓扑",
    "view.validation": "校验",
    "settings.label": "设置",
    "settings.language": "语言",
    "settings.english": "英语",
    "settings.chineseSimplified": "简体中文",
    "auth.loading": "权限...",
    "auth.role.manager": "管理员",
    "auth.role.editor": "编辑者",
    "auth.role.viewer": "查看者",
    "dataHall.selector": "数据厅",
    "dataHall.fallback": "数据厅",
    "dataHall.summary": "数据厅概览",
    "common.loading": "正在加载 {{target}}",
    "common.close": "关闭",
    "common.filter": "筛选",
    "common.rows": "Rows",
    "common.go": "跳转",
    "common.map": "地图",
    "common.count": "数量",
    "common.finding": "问题",
    "common.examples": "示例",
    "common.status": "状态",
    "common.group": "分组",
    "common.type": "类型",
    "common.port": "端口",
    "common.side": "侧",
    "common.model": "型号",
    "common.normalized": "规范化",
    "common.noFindings": "无问题。",
    "common.noExamples": "没有可显示的行示例。",
    "viewer.scale": "缩放",
    "viewer.tableScale": "表格文字",
    "cabinet.details": "机柜详情",
    "cabinet.cabinets": "机柜",
    "cabinet.category": "类型",
    "cabinet.categories": "类别",
    "cabinet.group": "分组",
    "cabinet.unassigned": "未分配",
    "cabinet.status": "状态",
    "cabinet.constructionPhase": "施工阶段",
    "cabinet.rackUnits": "机柜 U 位",
    "cabinet.devices": "设备",
    "cabinet.ports": "端口",
    "cabinet.cables": "线缆",
    "cabinet.types": "机柜类型",
    "bulkStatus.setStatus": "设置状态",
    "bulkStatus.chooseStatus": "设置 {{count}} 个已选对象...",
    "map.cabinetMap": "机柜地图",
    "portLayout.title": "Port Layout",
    "portLayout.mode": "Map mode",
    "portLayout.eyebrow": "Device Port Layout",
    "portLayout.front": "Front",
    "portLayout.back": "Back",
    "portLayout.noDevice": "No device selected",
    "portLayout.selectDevice": "Select a device to view its port layout.",
    "portLayout.noBackPorts": "No back-side port layout configured.",
    "map.size": "地图大小",
    "map.size.xsmall": "特小",
    "map.size.small": "小",
    "map.size.medium": "中",
    "map.size.large": "大",
    "map.size.xlarge": "特大",
    "rack.units": "机柜 U 位",
    "rack.unitLabel": "U{{unit}}",
    "device.portsCount": "{{count}} 个端口",
    "device.statusAndPorts": "{{status}} / {{count}} 个端口",
    "device.viewPortLayout": "Port layout",
    "connections.connectedDevices": "已连接设备",
    "connections.connectedDevicesCount": "{{count}} 个已连接设备",
    "connections.connectedCabinets": "已连接机柜",
    "connections.connectedCabinetsCount": "{{count}} 个已连接机柜",
    "connections.emptyCabinets": "选择机柜后会显示已连接机柜。",
    "connections.dataHallCables": "数据厅线缆",
    "connections.insideDataHall": "{{dataHall}} 内部",
    "connections.intraDataHall": "两端都在此数据厅内的线缆",
    "connections.totalExternal": "外部总计",
    "connections.totalExternalSubtitle": "从此数据厅跨接到其他数据厅的全部线缆",
    "connections.totalExternalCabinetSubtitle": "从此机柜连接到其他机柜的全部线缆",
    "connections.totalExternalDeviceSubtitle": "从此设备连接到其他机柜内设备的全部线缆",
    "connections.selectedCabinets": "{{count}} 个已选机柜",
    "connections.selectedDevices": "{{count}} 个已选设备",
    "connections.intraCabinetSelection": "已选机柜内部的设备到设备线缆",
    "connections.intraDeviceSelection": "已选设备之间的线缆",
    "connections.betweenSelectedCabinets": "已选机柜之间",
    "connections.selectedCabinetPeerSubtitle": "已选机柜之间的跨柜线缆",
    "connections.totalExternalCabinetSelectionSubtitle": "从已选机柜连接到未选机柜的全部线缆",
    "connections.totalExternalDeviceSelectionSubtitle": "从已选设备连接到未选设备的全部线缆",
    "connections.toDataHall": "到 {{dataHall}}",
    "connections.crossDataHall": "跨数据厅线缆",
    "connections.cableCount": "{{count}} 根线缆",
    "connections.deviceCount": "{{count}} 个设备",
    "connections.insideCabinet": "{{cabinetUid}} 内部",
    "connections.intraCabinet": "此机柜内的设备到设备线缆",
    "connections.intraDevice": "此机柜内的已连接设备",
    "connections.targetCabinetRu": "机柜 {{cabinetUid}} / RU {{rackUnit}}",
    "connections.viewCables": "查看线缆",
    "connections.expandPeers": "显示对端目标（{{count}}）",
    "connections.collapsePeers": "隐藏对端目标",
    "completion.text": "{{completed}}/{{total}} 完成",
    "completion.aria": "线缆完成度",
    "cable.details": "线缆详情",
    "cable.routeTitle": "{{source}} 到 {{target}}",
    "cable.countVisible": "{{visible}} / {{total}} 根线缆",
    "cable.closeDetails": "关闭线缆详情",
    "cable.column.type": "类型",
    "cable.column.status": "状态",
    "cable.column.group": "分组",
    "cable.column.aPort": "A 端端口",
    "cable.column.zPort": "Z 端端口",
    "cable.column.aOptic": "A 端光模块",
    "cable.column.zOptic": "Z 端光模块",
    "cable.column.uid": "线缆 ID",
    "cable.column.lengthMeters": "长度 (m)",
    "cable.column.progress": "进度",
    "cable.column.constructionPhase": "施工阶段",
    "cable.column.note": "备注",
    "cable.filterAll": "全选",
    "cable.filterNone": "全不选",
    "cable.filterHiddenOn": "{{count}} 个隐藏值已选中",
    "cable.filterHiddenOff": "{{count}} 个隐藏值已排除",
    "cable.phaseType.single_percent": "单任务",
    "cable.phaseType.parallel_percent": "并行任务",
    "cable.phaseType.enum_state": "枚举状态",
    "cable.phaseName.routing_dress": "路径理线",
    "cable.phaseName.termination": "端接",
    "cable.phaseName.cabinet_dress": "柜内理线",
    "cable.phaseName.final_result": "最终结果",
    "cable.phaseName.preparation": "准备",
    "cable.phaseName.dress_termination": "理线 / 端接",
    "cable.phaseName.validation": "验证",
    "cable.step.preparation": "准备",
    "cable.step.ordered": "已订购",
    "cable.step.received": "已接收",
    "cable.step.labeled": "已贴标",
    "cable.step.bundled": "已打捆",
    "cable.step.pulled": "已敷设",
    "cable.step.routing_dress": "路径理线",
    "cable.step.a_side": "A 端",
    "cable.step.z_side": "Z 端",
    "cable.step.not_terminated": "未端接",
    "cable.step.terminated": "已端接",
    "cable.step.dressed": "已理线",
    "cable.step.validation": "验证",
    "cable.step.validated": "已验证",
    "cable.step.failed": "未通过",
    "cable.step.broken": "损坏",
    "cabinet.termination": "端接",
    "cabinet.dress": "理线",
    "map.display": "地图显示",
    "map.displayText": "类型",
    "map.displayType": "类型",
    "map.displayTermination": "端接",
    "map.displayDress": "理线",
    "map.loadingData": "正在加载数据",
    "validation.qa": "QA",
    "validation.results": "校验结果",
    "validation.portCollisions": "端口冲突",
    "validation.modelMismatches": "型号不一致",
    "validation.formatIssues": "格式问题",
    "validation.deviceModelMismatches": "设备型号不一致",
    "validation.deviceModelFormatIssues": "设备型号格式问题",
    "validation.device": "设备",
    "validation.models": "型号",
    "validation.findingDetail": "问题详情",
    "validation.closeDetail": "关闭校验详情",
    "validation.deviceName": "设备名",
    "validation.aModel": "A 端型号",
    "validation.zModel": "Z 端型号",
    "validation.classification.model_mismatch": "型号不一致",
    "validation.classification.trivial_format_issue": "格式问题",
    "lifecycle.unknown": "未知",
    "lifecycle.not_constructed": "未建设",
    "lifecycle.not_installed": "未安装",
    "lifecycle.not_powered": "未上电",
    "lifecycle.installed": "已安装",
    "lifecycle.powered": "已上电",
    "lifecycle.active": "运行中",
    "lifecycle.not_planned": "不计划安装",
    "constructionPhase.Management & Ethernet": "管理与以太网",
    "constructionPhase.RoCE": "RoCE",
    "cableStatus.Cable Is Ran: Complete": "线缆已敷设：完成",
    "cableStatus.Cable Is Ran: Not Terminated": "线缆已敷设：未端接",
    "cableStatus.Cable Not Run": "线缆未敷设",
    "cableStatus.Unknown": "未知",
    "cableProgressState.not_started": "未开始",
    "cableProgressState.complete": "已完成",
    "cableProgressState.blocked": "受阻",
    "cableProgressState.not_applicable": "不适用",
    "cableProgressStep.purchased": "已采购",
    "cableProgressStep.received": "已收货",
    "cableProgressStep.categorized_stored": "已分类入库",
    "cableProgressStep.labeled": "已贴标",
    "cableProgressStep.bundled_on_ground": "地面预捆扎",
    "cableProgressStep.pulled": "已拉线",
    "cableProgressStep.dressed": "已理线",
    "cableProgressStep.a_side_terminated": "A 端已端接",
    "cableProgressStep.z_side_terminated": "Z 端已端接",
    "cableProgressStep.a_side_dressed_in_cabinet": "A 端柜内已理线",
    "cableProgressStep.z_side_dressed_in_cabinet": "Z 端柜内已理线",
    "cableProgressStep.validated": "已验证",
    "cableProgressStep.broken": "损坏",
  },
};

const I18nContext = createContext<I18nContextValue | null>(null);

export function I18nProvider({ children }: { children: React.ReactNode }) {
  const [locale, setLocaleState] = useState<Locale>(() => {
    const stored = window.localStorage.getItem(STORAGE_KEY);
    return stored === "zh-CN" || stored === "en" ? stored : "en";
  });

  const value = useMemo<I18nContextValue>(() => {
    function t(key: string, params: TranslationParams = {}) {
      const template = TRANSLATIONS[locale][key] ?? TRANSLATIONS.en[key] ?? key;
      return Object.entries(params).reduce(
        (text, [paramKey, paramValue]) => text.replaceAll(`{{${paramKey}}}`, String(paramValue)),
        template,
      );
    }

    function setLocale(nextLocale: Locale) {
      window.localStorage.setItem(STORAGE_KEY, nextLocale);
      setLocaleState(nextLocale);
    }

    const numberFormatter = new Intl.NumberFormat(locale);
    const percentFormatter = new Intl.NumberFormat(locale, {
      maximumFractionDigits: 0,
      style: "percent",
    });

    return {
      locale,
      setLocale,
      t,
      formatNumber: (value: number) => numberFormatter.format(value),
      formatPercent: (value: number) => percentFormatter.format(value),
      formatConstructionPhase: (phase: string) => t(`constructionPhase.${phase}`),
      formatLifecycleStatus: (status: string) => t(`lifecycle.${status}`),
      formatCableStatus: (status: string) => t(`cableStatus.${status || "Unknown"}`),
      formatCableProgressStep: (step: string) => {
        const formatted = t(`cableProgressStep.${step}`);
        return formatted === `cableProgressStep.${step}` ? t(`cable.step.${step}`) : formatted;
      },
      formatCableProgressState: (state: string) => t(`cableProgressState.${state}`),
      formatCableProgressPhaseName: (phase: string) => t(`cable.phaseName.${phase}`),
      formatCableProgressPhaseType: (phaseType: string) => t(`cable.phaseType.${phaseType}`),
      formatValidationClassification: (classification: string) =>
        t(`validation.classification.${classification}`),
    };
  }, [locale]);

  return <I18nContext.Provider value={value}>{children}</I18nContext.Provider>;
}

export function useI18n() {
  const context = useContext(I18nContext);
  if (!context) throw new Error("useI18n must be used inside I18nProvider");
  return context;
}


